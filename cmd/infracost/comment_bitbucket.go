...
func commentBitbucketCmd(ctx *config.RunContext) *cobra.Command {
  ...
  delete-and-new    Delete previous matching comments and create a new comment`)
	...
	cmd.Flags().String("bitbucket-server-url", "", "Bitbucket Server URL")
	cmd.Flags().String("bitbucket-token", "", "Bitbucket access token")
	...
	cmd.Flags().String("tag", "", "Customize text used to detect comments posted by Infracost. It is placed at the bottom of a comment")
